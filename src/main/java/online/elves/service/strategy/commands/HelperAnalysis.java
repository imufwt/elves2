...
public class HelperAnalysis extends CommandAnalysis {
    ...
    public void process(String commandKey, String commandDesc, String userName) {
        Fish.sendMsg("亲爱的 @" + userName +" "+ CrLevel.getCrLvName(userName) +" "+ " : 你好呀~\n" +
                ...
                "* `凌  7` 或 `凌  弹幕巨头` 统计 **发送弹幕** 前 10名\n" +

                "* `凌  10` 或 `凌  兑换日榜` 统计 **兑换鱼翅积分消耗日榜** 前 10名\n" +
                "* `凌  11` 或 `凌  兑换周榜` 统计 **兑换鱼翅积分消耗周榜** 前 10名\n" +
                "* `凌  12` 或 `凌  兑换月榜` 统计 **兑换鱼翅积分消耗月榜** 前 10名\n" +
                "* `凌  13` 或 `凌  兑换年榜` 统计 **兑换鱼翅积分消耗年榜** 前 10名\n" +
                "* `凌  14` 或 `凌  兑换总榜` 统计 **兑换鱼翅积分消耗总榜** 前 10名\n" +
                ...
                "* `凌  25` 或 `凌  散财日榜` 统计 **发红包金额日榜** 前 10名\n" +
                "* `凌  26` 或 `凌  散财周榜` 统计 **发红包金额周榜** 前 10名\n" +
                "* `凌  27` 或 `凌  散财月榜` 统计 **发红包金额月榜** 前 10名\n" +
                "* `凌  28` 或 `凌  散财年榜` 统计 **发红包金额年榜** 前 10名\n" +
                "* `凌  29` 或 `凌  散财总榜` 统计 **发红包金额总榜** 前 10名\n" +


                "* `凌  30` 或 `凌  赌圣日榜` 统计 **猜拳红包赢积分日榜** 前 10名\n" +
                "* `凌  31` 或 `凌  赌圣周榜` 统计 **猜拳红包赢积分周榜** 前 10名\n" +
                "* `凌  32` 或 `凌  赌圣月榜` 统计 **猜拳红包赢积分月榜** 前 10名\n" +
                "* `凌  33` 或 `凌  赌圣年榜` 统计 **猜拳红包赢积分年榜** 前 10名\n" +
                "* `凌  34` 或 `凌  赌圣总榜` 统计 **猜拳红包赢积分总榜** 前 10名\n" +


                "* `凌  35` 或 `凌  赌狗日榜` 统计 **猜拳红包输积分日榜** 前 10名\n" +
                "* `凌  36` 或 `凌  赌狗周榜` 统计 **猜拳红包输积分周榜** 前 10名\n" +
                "* `凌  37` 或 `凌  赌狗月榜` 统计 **猜拳红包输积分月榜** 前 10名\n" +
                "* `凌  38` 或 `凌  赌狗年榜` 统计 **猜拳红包输积分年榜** 前 10名\n" +
                "* `凌  39` 或 `凌  赌狗总榜` 统计 **猜拳红包输积分总榜** 前 10名\n" +
                
                "* `凌  报时` 看看节假日, 不用返回主页啦. 抄的阿达的代码. 嘻嘻~\n" +
                ...
                "* `凌  背包` 查询您的`鱼丸`与`鱼翅`数量. \n" +
                "* `凌  当前活跃度/活跃度/活跃` 查询您的当前活跃度(活跃度计算基于您在聊天室的发言,无法感知其余操作, 一定有误差切莫较真) \n" +
                "* `凌  运势/今日运势` 查询今日运势. 程序员只相信程序生成的黄历.来看看吧~ \n" +
                "* `凌  摸鱼历/摸鱼/日历/鱼历` 摸鱼日历, 看看图一乐~ \n" +
                "* `凌  捞鱼丸` 每天中午11:30, 下午17:30 鱼排上空准时出现鱼丸雨.所有渔民可以使用`捞鱼丸`命令获得随机[0,10]个`鱼丸`, 限时一分钟.~ \n" +
                "* `凌  赠送鱼翅 username_xxx` 可以相互赠送咯.`username`是对方用户名.`xxx`是数量乱写/不写都是默认1. 预计增加小冰好感度比例 1:10~ \n" +
                "* `凌  赠送鱼丸 username_xxx` 可以相互赠送咯.`username`是对方用户名.`xxx`是数量乱写/不写都是默认1. 预计增加小冰好感度比例 1:1~ \n" +
                "* `凌  V50` 疯狂星期四, 冲鸭~ (感谢老王(巫昂)发起活动, 感谢王总(wuang)赞助!).~ \n" +
                "* `凌  决斗/对线` 精灵发送猜拳红包, 试试你的手气(决斗会写你的名字, 对线不会~~听说小智是赌王?~~).~ \n" +
                "* `凌  15` 或 `凌 今日水分` 看看你今天水了多少条.~ \n" +
                "* `凌  探路者` 看看[迷宫游戏](https://maze.hancel.org/)排行榜. 暴多积分等你来拿~ \n" +
